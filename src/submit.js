...
export const SubmitButton = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.error(error);
      toast.error('Failed to submit pipeline to backend. Is the backend running?');
    }
  };
    ...
    <>
      ...
      <div className="flex items-center justify-center p-4">
        <button 
          ...
          className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold py-2 px-7 rounded-lg shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50 text-lg tracking-wide"
        >
          Submit Pipeline
        </button>
      </div>
